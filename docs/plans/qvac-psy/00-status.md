...
## Open evidence
The [actual desktop workflow](../../../artifacts/evidence/desktop-workflow.json) passed ten renderer steps with linked real GPU extraction/drafting, corrected-source prompt, exact approval, encrypted reload, history isolation and lock purge. The coordinator and dedicated performance owner independently validated the receipt. Root tests: 29 passed, no failures/skips; desktop gate and mobile TypeScript check passed. The full release gate deliberately fails without a separate physical Fold receipt and requires ordered trusted UI observations linked to canonical records.
