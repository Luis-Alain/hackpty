...
The reviewed [Android build record](../../apps/mobile/evidence/lifecycle-preparation.json) identifies the final APK; installation remains unverified. Preserve SDK 0.18.2 and team ancestry 21f7f40736c201f8d3c14a36ced4cdb426639122. Available agent cells used the selected inherited runtime; no GLM, Kimi or Fable execution is claimed.
...
- Run `./apps/mobile/scripts/install-verified-android.ps1` to install the latest locally built self-contained Release APK and compare its SHA-256 with the installed base APK. Use `-VerifyOnly` to inspect an existing installation. Do not uninstall or clear app data; pending captures and Android Keystore keys must survive.
...
5. Return to the approved patient, lock, verify renderer purge, unlock with the private passphrase and verify the exact immutable approved text reloads.
