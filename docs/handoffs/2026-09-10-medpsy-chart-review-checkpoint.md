...
OCR: both bitmap fixtures extracted with complete native metrics (run `20260910T0230Z`); offline scoring WER 0.056, CER 0.035; the summary is pending a rerun with a fresh id after the assertion fix. The real-runtime desktop integration check for the chart review passed 1/1 with MedPsy (evidence under the session workspace, to be published as synthetic evidence in the next checkpoint).
